...
pub struct PriceData {
    ...
    pub timestamp: i64,
    /// Size available at the best ask for the UP token
    pub up_ask_size: f64,
    /// Size available at the best ask for the DOWN token
    pub down_ask_size: f64,
    /// Individual spread for up side (up_ask - up_bid)
    pub up_spread: f64,
...
pub fn create_price_data(
    ...
    let down_ask = down_snapshot
        ...
        .map(|a| a.price)
        .unwrap_or(0.0);
    let down_ask_size = down_snapshot
        .and_then(|s| s.asks.first())
        .map(|a| a.size)
        .unwrap_or(0.0);
...
pub struct PriceMonitor {
    ...
    arbitrage_history: HashMap<String, VecDeque<ArbitrageDetection>>,
}
...
impl PriceMonitor {
    ...
    pub fn record_arbitrage(&mut self, coin: &str, price_data: &PriceData) {
        let history = self.arbitrage_history.entry(coin.to_string()).or_insert_with(VecDeque::new);
        history.push_back(ArbitrageDetection {
    ...
    pub fn get_arbitrage_history(&self, coin: &str) -> Vec<ArbitrageDetection> {
        self.arbitrage_history.get(coin).map(|d| d.iter().cloned().collect()).unwrap_or_default()
    }
    ...
    pub fn add_to_history(&mut self, coin: &str, price_data: PriceData, _env: &Env) {
        let history = self.price_history.entry(coin.to_string()).or_insert_with(VecDeque::new);
    ...
    pub fn get_price_history(&self, coin: &str) -> Vec<PriceData> {
        self.price_history.get(coin).map(|d| d.iter().cloned().collect()).unwrap_or_default()
    }
}

fn format_timestamp(timestamp: i64) -> String {
    let dt = DateTime::from_timestamp_millis(timestamp).unwrap_or_else(|| Utc::now());
    dt.format("%H:%M:%S UTC").to_string()
}
